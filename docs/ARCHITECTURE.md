...
每份基础 STL 不再被理解成一个临时模板，而是一个构型族的几何种子。构型族注册表统一声明稳定 ID、STL 种子、参数 schema、生成器、预设与版本；三孔接口、底座联动、实时预览和导出是跨构型共享能力。

当前注册了：

- `source`：由主体 STL 与底座 STL 组成的原始夹爪构型；
- `finray`：程序生成的 Fin-Ray 柔顺构型；
- `arc_wrap`：宽面薄身、连续弧槽与圆钝止挡组成的包覆构型；
- `precision_tip`：紧凑楔身与局部平直窄尖组成的精密捏取构型。

四个构型共享同一 Robotiq 三孔安装契约、底座联动、预览、检查和导出链路。接口参数属于当前构型的设计状态：主体和底座必须同步，但不同构型可以保留适合自身轮廓的默认孔距。原始与 Fin-Ray 使用原始 STL 的 `30.071797 mm` 横向轴距，两个原创紧凑构型默认使用 `22 mm`。

新增流程见 [`ADDING_A_FAMILY.md`](ADDING_A_FAMILY.md)。
...
| `backend/families.py` | 构型族注册契约与目录 |
| `backend/geometry_families.py` | 独立程序构型的参数与活动主体生成器 |
| `backend/schema.py` | 参数范围、分组、默认值与模板 |
| `backend/model.py` | 几何领域内核：网格生成、接口联动和质量报告 |

## 修改原则

1. 新的基础 STL 应注册成独立构型族；同一构型的常用参数组合才属于预设。
2. 新参数先加入该构型的 schema，再实现真实网格变化，最后补参数极值与组合测试。
3. 三孔参数必须位于顶层 `design.interface`，不能放入左右手指各自的参数中。
4. 主体与底座必须由同一个 `Design` 求值；固定单孔与 Robotiq 连接特征不得漂移。
5. 前端预览只能展示后端返回的真实 STL，不能用视觉缩放冒充参数化结果。
6. 只有 `npm test` 全部通过的版本才进入稳定分支。
